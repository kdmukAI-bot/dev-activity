...
_EXCLUDED_FILE_SUFFIXES = {
    ...
    ".npy", ".npz",
    ".bin",
    # Dependency lockfiles: regenerated by package managers, not hand-authored.
    # Covers Cargo.lock, poetry.lock, uv.lock, yarn.lock, composer.lock,
    # Gemfile.lock, Pipfile.lock, flake.lock, ESP-IDF dependencies.lock, bun's
    # binary bun.lockb, etc. (npm/pnpm/NuGet lockfiles carry .json/.yaml names —
    # see _EXCLUDED_FILE_BASENAMES.)
    ".lock", ".lockb",
...
# Exact basenames (case-insensitive) of machine-generated config/build artifacts
# that tooling regenerates rather than a human authoring line-by-line. Unlike the
# suffix set, these are matched on the *whole* basename because the generated file
# carries no telltale extension — ESP-IDF writes a fully-expanded `sdkconfig`
# (thousands of lines, header: "Automatically generated file. DO NOT EDIT.") out
# of the hand-authored `sdkconfig.defaults`. Matching the exact basename means the
# authored inputs (`sdkconfig.defaults`, `sdkconfig.board`, `sdkconfig.ci`,
# `sdkconfig.landscape`, `sdkconfig.combined.in`, ...) still count as real work.
_EXCLUDED_FILE_BASENAMES = {
    "sdkconfig",          # ESP-IDF generated full config (expanded from defaults)
    "sdkconfig.old",      # menuconfig backup of the previous generated config
    "sdkconfig.combined", # builder-merged generated config
    # Lockfiles whose name carries a data extension (.json/.yaml), so the
    # ".lock" suffix rule can't target them without also excluding real source.
...
def _is_excluded_file(path: str) -> bool:
    ...
    if basename in _EXCLUDED_FILE_BASENAMES:
        return True
